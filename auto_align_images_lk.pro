...
function auto_align_images_func,pqin,init=init,noplot=keynoplot
  ...
  ;    ocontour2,sitest,level=clevels,bin=bin,c_color=!d.table_size-1 does not work in idl 8
  ;modified lk: 
           contour,sitest,level=clevels,c_color=!d.table_size-1,yrange=[0,sny-1],$
            xrange=[0,snx-1],xs=5,ys=5,/noer,position = [0,0,snx*bin,sny*bin],/device
...
function auto_align_images_lk,timagein,rimagein,pin,qin,pout,qout, $
...
if NOT quiet AND NOT noplot then begin  ; Optionally display the alignment
 ...
 ;new lk:
   contour,itest,level=clevels,c_color=!d.table_size-1,$
   xrange=[0,rnx-1],xs=5,ys=5,/noer,position = [0,0,rnx*bin,rny*bin],/device,yrange=[0,rny-1]   
;   ocontour2,itest,level=clevels,c_color=!d.table_size-1,bin=bin does
;   not work in idl 8.0
