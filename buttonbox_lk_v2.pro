...
; mod Jan 2014 for pointer instead of common
; CATEGORY:
;
;  Utility, Widgets
;
...
PRO buttonbox_lk_v2_CenterTLB, tlb
...
PRO buttonbox_lk_v2_Event, event, info=info
common point, ptr
...
npolcal = n_elements(*info.poldir)+1
if (nr2 ge 0 and nr2 lt npolcal-1) then begin
(*ptr).text = strmid((*info.poldir)[nr2],14,15,/rev)

Widget_Control, event.top, /Destroy
endif

;user selects 'n/a'
if nr2 eq npolcal-1 then begin
(*ptr).text = 'N/A'
Widget_Control, event.top, /Destroy
endif
...
FUNCTION buttonbox_lk_v2,Cancel=cancel, Group_Leader=groupleader, info=info
...
npolcal = n_elements(*info.poldir)+1 ;140327 increasy by 1 for n/a option
...
for i=0,npolcal-2 do textid = widget_button(tlb, value=strmid((*info.poldir)[i],14,15,/rev),uvalue=i)
;label last position as n/a
i=npolcal-1
textid = widget_button(tlb, value='N/A',uvalue=i)
