...
pro setpts_lk,p,ima,imr,trima=trima,plotonly=pflg,noscale=tvflg,append=aflg, $
           key=ttyp
...
;       LK 2011 remove the stupid fixed window size which blocks the
;       whole desktop.../noscale doesn't work anyway
;       added xrange for contour -> otherwise compressed
;       image in idl 8.0
;-
...
; create window if necessary (with maximum screen size!)
if (!d.window lt 0) or (!d.x_size ne xmax) or (!d.y_size ne ymax) then $
   window,/free,retain=2,title='lk, left : image   right : reference', $
          xsize=xmax,ysize=ymax
erase

; display reference image (imr)
sizr = size(imr) ;512x512 for nb
zomr=float([ymax,ymax])/max(sizr(1:2));1,1
rnx = long(sizr(1)*zomr(0)+0.5); 512
rny = long(sizr(2)*zomr(1)+0.5); 512
...
anx = long(siza(1)*zoma(0)+0.5); 256
any = long(siza(2)*zoma(1)+0.5); 512
if keyword_set(tvflg) then $
...
;if the program stalls here (and cursor has no effect) on a mac, then
;try a solution from here:
;http://www.exelisvis.com/Support/HelpArticlesDetail/TabId/219/ArtMID/900/ArticleID/3947/3947.aspx
;enabling "Click-through Inactive Windows" in X11 Preferences solved
;my issue.
   repeat begin
