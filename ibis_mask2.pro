...
;works on dual beam images

FUNCTION ibis_mask2,img,cut=cut
...
;--- in x direction ---
  ...
  szcut = size(tmp)
    ...
    if szcut[1] le 2*cut then begin
       ...
       ;for dual beam:
          pos = where(tmp[1:*]-tmp[0:*] gt 1)
          if pos[0] eq -1 then goto,notinside2 ;not 2 fovs
           msk[tmp[pos[0]]-cut:tmp[pos[0]],j] = 0 ;edge larger at lower end of image
           msk[tmp[pos[0]+1]:tmp[pos[0]+1]+cut,j] = 0 ;edge larger at lower end of image
    endelse
  notinside2:
  endfor
endif
