...
PRO IBIS_GET_POLCALCURVE_v2, lambda, path, tpfile, region, dfilen, sfile, path1, $
...
;;***************************************************************
     ...
     npos = round(f1+f2+f3+f2)/28.  ;number of table pos
         ...
         FOR k = 0, nmeas - 1 DO BEGIN ;5 wl points for 6302
             ...
             IF (npol eq 6) THEN BEGIN 
                 ...
                 IF flagncam NE 1 THEN BEGIN 
                     ...
                     FOR l=0,5 DO BEGIN
                         azim_k[l]    = (FIND_IBIS_TEL_PARAMS(telparams,(nb_expos[k*npol+l]).time_string, /NB)).azim
                         elev_k[l]    = (FIND_IBIS_TEL_PARAMS(telparams,(nb_expos[k*npol+l]).time_string, /NB)).elev
                         table_k[l]   = (FIND_IBIS_TEL_PARAMS(telparams,(nb_expos[k*npol+l]).time_string, /NB)).table
                         llevel_k[l]  = (FIND_IBIS_TEL_PARAMS(telparams,(nb_expos[k*npol+l]).time_string, /NB)).llevel
                     ENDFOR
                     
                     pc_azim[i,k]   = MEAN(azim_k) ;i is 112 (4*28), k is wavelengths
                     pc_elev[i,k]   = MEAN(elev_k)
                     pc_table[i,k]  = MEAN(table_k)
                     pc_llevel[i,k] = MEAN(llevel_k) 

                     restore,sfile,/ve ;restores cu_settings.sav, which contain the pt4 parameters for old data
                                ;i.e. if anything went wrong or if the
                                ;angles changed, you're screwed  

                     
                      pc_pt4_pstg[i,k] = IN_BEAM_LIN_POL[i]
                      pc_pt4_pol[i,k]  = CAL_SETTINGS_LIN_POL[i]
                      pc_pt4_rstg[i,k] = IN_BEAM_RETARDER[i]
                      pc_pt4_ret[i,k]  = CAL_SETTINGS_RETARDER[i]
...
;;***************************************************************
     ...
     nbpolcal       = TOTAL(nbpolcal,4)/nmeas
...
;;***************************************************************
     ...
     IF KEYWORD_SET(SAVEFILE) THEN BEGIN
        ...
        SAVE, /VERB, /COMP, FILENAME = savefile, $
          ...
          pc_pt4_pstg, pc_pt4_pol, pc_pt4_rstg, pc_pt4_ret, pc_pt4_dstg
